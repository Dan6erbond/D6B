...
def find(options, search, return_all=False, coverage_multiplier=0.02975):
    ...
    for option in options:
        ...
        typo_tolerance = len(key) / 4
        typos = len(construct) - len(key)

        if coverage < max_coverage and not tag_match and not tag_occurence and not typos > typo_tolerance: # and accuracy < 0.2:
            continue

        cat = 7
        if match == 1:
            cat = 0
        elif search_in_key == 2 or typos <= typo_tolerance:
            cat = 1
        elif starts_with == 2:
            cat = 2
        elif tag_match:
